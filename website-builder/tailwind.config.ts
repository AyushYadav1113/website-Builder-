...
// Extend Tailwind's config type to allow `safelist`
type SafeListEntry =
  | string
  | {
      pattern: RegExp;
...
interface ConfigWithSafelist extends TailwindConfig {
    ...
    extend: {
      ...
      keyframes: {
        ...
        "automation-zoom-in": {
          ...
          "100%": { transform: "translateY(0px) scale(1)" }, // fixed typo
        },
      },
      animation: {
        scroll:
...
// Cast back to TailwindConfig for `withUt`
export default withUt(config as TailwindConfig);
